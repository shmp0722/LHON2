...
for i = 1:length(subDir)
    ...
    cd(fgDir)
    % load fg
    fgf = dir('fg_OCF_Top50K_fsV1V2_3mm_lh_V1_smooth3mm_lh_V2_smooth3mm_rh_V1_smooth3mm_rh_V2_smooth3mm_*.pdb');
...
%     fgf = dir('*fg_OCF_Top50K*+fs_CC.pdb');
%     fg = fgRead(fgf.name);
...
for i = 1:length(subDir)
    ...
    % load fg
    fgf = dir('fg_OCF_Top50K_fsV1V2_3mm_lh_V1_smooth3mm_lh_V2_smooth3mm_rh_V1_smooth3mm_rh_V2_smooth3mm_*+fs_CC.pdb');
    fg = fgRead(fgf.name);
    % run
...
for i = 1:length(subDir)
    ...
    % load fg
    fgf = 'OCFV1V2Not3mm_MD4.pdb';
    fg = fgRead(fgf);
    fg = SO_AlignFiberDirection(fg, 'LR');
    fgWrite(fg,[fg.name 'Al.pdb'],'pdb')
end
