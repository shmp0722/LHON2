...
% core. Let's dop it for a new tract.
 ...
 if i <= sum(afq.sub_group);
     saveas(gcf,['LHON' num2str(i) '_FA_spf1000'],'fig');
 else i > sum(afq.sub_group);
     saveas(gcf,['Ctl' num2str(i-sum(afq.sub_group)) '_FA_spf1000'],'fig');
 end
 close;
 ...
 set(gcf,'Color','w','InvertHardCopy','off','PaperPositionMode','auto');
 if i <= sum(afq.sub_group);
         saveas(gcf,['LHON' num2str(i) '_FA_tube3000'],'fig');
 else i > sum(afq.sub_group);
         saveas(gcf,['Ctl' num2str(i-sum(afq.sub_group)) '_FA_tube3000'],'fig');
 end
 close;
end
...
% AFQ_RenderFibers(fgL,'color',rgbL);
