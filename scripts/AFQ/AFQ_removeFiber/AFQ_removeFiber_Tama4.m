...
for ii=1:length(subs)
    ...
    fgDir = fullfile(homeDir,subs{ii},'dwi_2nd','fibers','conTrack','Top200000');
...
%         for i=2:3;
            ...
            tmp = strfind(fiberfile_pdb{ij}, '_');            
            fibername =sprintf('%s/%sMD%d.pdb',fgDir,fiberfile_pdb{ij}(tmp(1)+1:tmp(4)-1),2);
            savefile = fullfile(fgDir,fibername);
            
            % outputfibername = fullfile(fgDir, sprintf('%s%s-%d.pdb',name,fileidentifier,nFiber));
                        
            mtrExportFibers(fgclean,fgDir,savefile);
%         end
    end
end
